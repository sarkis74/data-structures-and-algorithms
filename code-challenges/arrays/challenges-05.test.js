...
const listFoods = (recipe) => {
    let regex = /\d+\s\w{1,}\s|\d\s\w{1,}-\w{1,}\s/;
    let result = [];
    // Solution code here...
    for(var i in recipe.ingredients) {
    if(recipe.ingredients[i].match(regex)) {
      let temp = recipe.ingredients[i].match(regex)
      result.push(recipe.ingredients[i].slice(temp[0].length))
    }
...
const splitFoods = (recipe) => {
  ...
  for(var i in recipe.ingredients) {
  if(recipe.ingredients[i].match(regex)) {
    temp = recipe.ingredients[i].match(regex)
    temp = (recipe.ingredients[i].split(temp[0]));
    temp+= counter;
    }
  result.push(temp.substring(1, temp.length))
  }
return result;
}
/* ------------------------------------------------------------------------------------------------
...
const removeEvenValues = (arr) => {
  ...
  arr.splice(0, arr.length - result.length)
}
