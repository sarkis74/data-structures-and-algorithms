...
const validatePhoneNumber = (phoneNumber) => {
    // Solution code here...
    let regex = /^(\(\d{3}\))\-?\s?\d{3}\s?\-?\d{4}$|^\d{3}\-?\s?\d{3}\s?\-?\d{4}$/;
...
const findTagNames = elements => {
  ...
  for(var i in elements) {
    tempArr.push(elements[i].split('<'));
    }
  tempArr.filter(arr => {
    arr.filter(value => {
      resultArr.push(value.match(regex));
      }) 
    })
    let final = resultArr.filter(value => {
      return value != null;
    })
    final = final.join('');
    final = final.match(/\/+\w+/g)
